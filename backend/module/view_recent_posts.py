...
def view_recent_posts(posts_collection, category):
    posts = list(posts_collection.find({"category": category}).sort("date", -1).limit(30))
    
    if not posts:
        return jsonify({"message": "해당 카테고리의 게시물이 없습니다."}), 404

    # 결과 포맷을 API 명세서와 일치시키기
    formatted_posts = [{"postId": str(post["_id"]), "userId": post["userId"]} for post in posts]
